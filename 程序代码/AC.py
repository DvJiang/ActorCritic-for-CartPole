...
class Critic:
    ...
    def fit(self, state, reward, state_):
        value = reward + self.model(state_) * self.gama
...
def main():
    ...
    for epi in range(200):
        ...
        for t in range(1000):
            ...
            if (epi >= 179 and epi <= 189):frames2.append(env.render(mode = 'rgb_array'))
            ...
            if (done and t >= 199): reward = 100
            rewards.append(reward)
            value = critic.fit(observation, reward, observation_)
            actor.fit(observation, action, value)
    ...
    display_frames_as_gif(frames2,'180-190-result')
